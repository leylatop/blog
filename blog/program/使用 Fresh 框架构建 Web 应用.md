...
核心代码在 [`routes\api\link.ts`](https://github.com/kuizuo/link-maker/blob/main/routes/api/link.ts) 下，将会生成 `/api/link` 接口，例如访问 [https://link-maker.deno.dev/api/link?q=https://leyla.top](https://link-maker.deno.dev/api/link?q=https://leyla.top 'https://link-maker.deno.dev/api/link?q=https://leyla.top') 你就可以得到如下 json 数据

```json
{
  "title": "愧怍的小站",
  "description": "Blog",
  "image": "https://leyla.top/img/logo.png",
  "url": "https://leyla.top"
}
```
...
至于说为何要用 query 而不是用 param，主要是因为 url 的缘故，比如说 `/api/link/https://leyla.top` 这个链接，这时 param 是解析不出 `https://leyla.top` 完整 url 的，除非 url 编码，但这对使用者来说就不是很好，于是就舍弃了 param 参数的方案。
